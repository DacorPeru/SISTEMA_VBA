...
IF COL_LENGTH('dbo.cen_usr','kdf_alg') IS NULL
  ALTER TABLE dbo.cen_usr
    ADD kdf_alg VARCHAR(32) NOT NULL
        CONSTRAINT DF_cen_usr_kdf DEFAULT('TSQL_STRETCH') WITH VALUES;
GO

-- (Opcional recomendado) CHECK para valores permitidos de kdf_alg
IF NOT EXISTS (SELECT 1 FROM sys.check_constraints WHERE name='CK_cen_usr_kdf')
    ALTER TABLE dbo.cen_usr
      ADD CONSTRAINT CK_cen_usr_kdf CHECK (kdf_alg IN ('TSQL_STRETCH','PBKDF2_SHA256'));
GO
